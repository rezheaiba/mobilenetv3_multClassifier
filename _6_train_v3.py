...
import _6_dataloader as dataloader
from utils import log

from _6_model_v3 import MobileNetV3
...
def main():
    ...
    image_path = os.path.join(data_root, "final8_mult")  # flower data set path
    batch_size = 128
    assert os.path.exists(image_path), "{} path does not exist.".format(image_path)

    train_dataset = dataloader.Loader(csv_path='label/train_label_6.csv',
                                      root=image_path,
    ...
    validate_dataset = dataloader.Loader(csv_path='label/test_label_6.csv',
                                         root=image_path,
    ...
    net = MobileNetV3(dilated=True, num_classes_1=2, num_classes_2=5, num_classes_3=2, num_classes_4=2, num_classes_5=2,
                      num_classes_6=2,dropout=0.5,
                      arch='mobilenet_v3_small', reduced_tail=True,
                      width_mult=0.5)
    # net.load_state_dict(torch.load('./weight-final-v3/model_v3_best.pth', map_location=device))
    # 
    # in_features = net.classifier2[3].in_features
    # out_features = 7
    # net.classifier2[3] = nn.Linear(in_features, out_features)
    # net.classifier3 = nn.Sequential()
    # net.classifier4 = nn.Sequential()
    net.to(device)
    ...
    epochs = 61
    best_acc = 0.0
    save_path_best = './weight-final-v3-6/model_v3_best.pth'
    save_path_newest = './weight-final-v3-6/model_v3_newest.pth'
    train_steps = len(train_loader)
    val_steps = len(validate_loader)
    prev_time = time.time()

    # creat logger 用来保存训练以及验证过程中信息
    logger_file = "./weight-final-v3-6/results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    for epoch in range(epochi, epochs):
        ...
        for step, data in enumerate(train_bar):
            ...
            loss5 = loss_function(logits['classifier5'], data['label5'].to(device))
            loss6 = loss_function(logits['classifier6'], data['label6'].to(device))
        ...
        with torch.no_grad():
            ...
            for step, val_data in enumerate(val_bar):
                ...
                predict_y_5 = torch.max(outputs['classifier5'], dim=1)[1]
                predict_y_6 = torch.max(outputs['classifier6'], dim=1)[1]
                ...
                acc5 += torch.eq(predict_y_5, val_data['label5'].to(device)).sum().item()
                acc6 += torch.eq(predict_y_6, val_data['label6'].to(device)).sum().item()
                ...
                loss5 = loss_function(outputs['classifier5'], val_data['label5'].to(device))
                loss6 = loss_function(outputs['classifier6'], val_data['label6'].to(device))
                val_loss = (loss1 + loss2 + loss3 + loss4 + loss5 + loss6) / 6
        ...
        val_accurate_5 = acc5 / val_num
        val_accurate_6 = acc6 / val_num
        
        print(
            '[epoch %d] train_loss: %.3f val_loss: %.3f val_accurate_1: %.3f val_accurate_2: %.3f val_accurate_3: %.3f val_accurate_4: %.3f val_accurate_5: %.3f val_accurate_6: %.3f lr: %f' %
            (epoch + 1, running_loss / train_steps, running_val_loss / val_steps, val_accurate_1, val_accurate_2,
             val_accurate_3, val_accurate_4, val_accurate_5, val_accurate_6, optimizer.param_groups[0]['lr']))
        ...
        # 保存准确率最高的权重
        val_accurate = (val_accurate_1 + val_accurate_2 + val_accurate_3 + val_accurate_4 + val_accurate_5 + val_accurate_6) / 6
