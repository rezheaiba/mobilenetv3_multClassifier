...
import _4_dataloader as dataloader
from utils import log

from _4_model_v3 import MobileNetV3
...
def main():
    ...
    train_dataset = dataloader.Loader(csv_path='label/train_label_4.csv',
                                      root=image_path,
    ...
    validate_dataset = dataloader.Loader(csv_path='label/test_label_4.csv',
                                         root=image_path,
    ...
    net = MobileNetV3(dilated=True, num_classes_1=2, num_classes_2=5,num_classes_3=2, num_classes_4=2,dropout=0.5,
    ...
    epochs = 30
    best_acc = 0.0
    save_path_best = 'weight-final-v3-4/model_v3_best.pth'
    save_path_newest = 'weight-final-v3-4/model_v3_newest.pth'
    train_steps = len(train_loader)
    val_steps = len(validate_loader)
    prev_time = time.time()

    # creat logger 用来保存训练以及验证过程中信息
    logger_file = "./weight-final-v3-4/results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
