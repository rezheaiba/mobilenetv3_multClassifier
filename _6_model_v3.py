...
class MobileNetV3(nn.Module):
    def __init__(
            ...
            num_classes_5: int = 1000,
            num_classes_6: int = 1000,
        ...
        self.classifier5 = nn.Sequential(
            ...
            nn.ReLU6(inplace=False),
            nn.Dropout(p=dropout, inplace=True),
            nn.Linear(last_channel, num_classes_5)
        )
        self.classifier6 = nn.Sequential(
            nn.Linear(lastconv_output_channel, last_channel),
            nn.ReLU6(inplace=False),
            nn.Dropout(p=dropout, inplace=True),
            nn.Linear(last_channel, num_classes_6)
        )
    ...
    def forward(self, x: Tensor) -> dict:
        ...
        return {
            ...
            'classifier5': self.classifier5(x),
            'classifier6': self.classifier6(x),
        }


def main():
    input = torch.randn([8, 3, 256, 256])
    model = MobileNetV3(dilated=True, num_classes_1=2, num_classes_2=7, num_classes_3=2, num_classes_4=2,
                        num_classes_5=2, num_classes_6=2, arch='mobilenet_v3_small',
    ...
    # torch.save(model.state_dict(), 'weight-final-v3/model_v3_mult.pth')
    print(model)
