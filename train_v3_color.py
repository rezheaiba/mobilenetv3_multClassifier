...
def main():
    ...
    batch_size = 64
    lr_epoch = 30000
    current_path = "final-8_mult"
    
    label_train_path = 'label_s/train_label.csv'
    label_test_path = 'label_s/test_label.csv'
    
    # freeze option
    freeze_classifier1 = True
    freeze_classifier2 = False
    ...
    image_path = os.path.join(data_root, current_path)  # flower data set path
    
    assert os.path.exists(image_path), "{} path does not exist.".format(image_path)

    train_dataset = dataloader.Loader(csv_path=label_train_path,
                                      root=image_path,
    ...
    validate_dataset = dataloader.Loader(csv_path=label_test_path,
                                         root=image_path,
    ...
    net = MobileNetV3(dilated=True, num_classes_1=2, num_classes_2=6,dropout=0.1,
    ...
    #net.classifier5 = nn.Sequential()
    #net.classifier6 = nn.Sequential()
    
    # freeze classifier1
    if freeze_classifier1:
        for k, v in net.named_parameters():
            if 'classifier1' in k:
                v.requires_grad = False
    if freeze_classifier2:
        for k, v in net.named_parameters():
            if 'classifier2' in k:
                v.requires_grad = False
    net.to(device)
    ...
    optimizer = optim.AdamW([{'params':params, 'initial_lr':1e-3}], lr=init_lr)
    #optimizer = optim.AdamW([{'params':params, 'initial_lr':1e-3}], lr=init_lr)
    #scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1,gamma=0.8, last_epoch=3)

    
    best_acc = 0.0
    save_path_best = 'weight-final-v3-color/model_v3_best.pth'
    save_path_newest = 'weight-final-v3-color/model_v3_newest.pth'
    train_steps = len(train_loader)
    val_steps = len(validate_loader)
    prev_time = time.time()

    # creat logger 用来保存训练以及验证过程中信息
    logger_file = "./weight-final-v3-color/results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
